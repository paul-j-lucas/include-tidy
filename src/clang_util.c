...
static void getScopedName_impl( CXCursor cursor, getCursorName_fn name_fn,
  ...
  if ( name == NULL || name[0] == '(' )
    goto done;  // Skip "(anonymous ...)" and "(unnamed ...)".
  ...
  if ( kind == CXCursor_DeclRefExpr ) {
    ...
    if ( kind == CXCursor_VarDecl ) {
      ...
      if ( !clang_Cursor_isNull( init_csr ) )
        return tidy_Cursor_getVarInit( init_csr );
    }
  }
  return expr_csr;
}
  ...
  switch ( kind ) {
    ...
    case CXCursor_VarDecl:
      if ( clang_isCursorDefinition( cursor ) ) {
        CXCursor const cls_csr = clang_getCursorSemanticParent( cursor );
        bool const is_out_of_line = !clang_equalCursors( parent, cls_csr );
        if ( is_out_of_line && pcls_csr != NULL )
          *pcls_csr = cls_csr;
...
bool tidy_Cursor_isSpellingEqualTo( CXCursor cursor, char const *value ) {
  ...
  char const *const s = clang_getCString( cxs );
  bool const        is_equal = s != NULL && strcmp( s, value ) == 0;
