...
static void add_compiler_include_paths( int *pargc, char const **pargv[],
  ...
  while ( getline( &line_buf, &line_cap, fcompiler ) != -1 ) {
    ...
    if ( str_ends_with( line_buf, FRAMEWORK_DIR, STRLITLEN( FRAMEWORK_DIR ) ) )
      continue;
#endif /* __APPLE__ */
 ...
 *    <tt>-I</tt><i>path</i>.
 *  + Same as above, but for also <tt>-cxx-system</tt>, <tt>-idirafter</tt>,
 *    <tt>-iquote</tt>, <tt>-isystem-after</tt>, <tt>--include-directory</tt>,
 *    and <tt>--include-directory-after</tt>.
...
void cli_options_init( int *pargc, char const **pargv[] ) {
  ...
  char const *const source_dir = path_dirname( tidy_source_path, dir_buf );
  ipath_add( source_dir );
