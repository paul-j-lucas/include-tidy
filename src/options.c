...
bool opt_comment_symbols_parse( char const *s ) {
  struct comment_map {
    char const   *comment_str;
    tidy_comment  comment;
  };
  typedef struct comment_map comment_map;

  static comment_map const COM_SYM_MAP[] = {
  ...
  FOREACH_ARRAY_ELEMENT( comment_map, m, COM_SYM_MAP ) {
    if ( strcasecmp( s, m->comment_str ) == 0 ) {
      opt_comment_symbols = m->comment;
      return true;
    }
  } // for
